...
That night, I started over. What emerged became my claude-code-tresor repository — 288 stars as of this writing, containing the 10-team structure, 8 autonomous skills, and 19 slash commands that turned chaos into something manageable.

Here’s what actually works.

Claude Code Agent Architecture: The 10-Team System
Most developers organize agents by task. “Code review agent.” “Documentation agent.” “Testing agent.” I tried this. It collapsed around agent number 30.
...
My solution: organize by domain instead. Each “team” owns a complete area of the codebase or workflow.

The 10 Teams
Engineering Teams (4):

Blue Team — Core application code
Cyan Team — Infrastructure and DevOps
Orange Team — External integrations and APIs
Yellow Team — Database and data processing
Support Teams (3):

Green Team — Quality assurance and testing
Red Team — Security auditing
Purple Team — Architecture and planning
Operational Teams (3):

Gray Team — Documentation and knowledge management
White Team — Performance monitoring and optimization
Black Team — Incident response and debugging
Why colors instead of names? Two reasons. First, colors are faster to type in commands (/blue-review vs /engineering-core-review). Second — and I learned this the hard way — colors force you to keep teams small. If you can't fit what the team does into a color association, you're probably trying to do too much.

Team Configuration Example
Here’s how I configure a team in my .claude/ directory:
...
For the hooks that power these agents, I’ve written a separate deep-dive — the handoff logic alone took three iterations to get right.

8 Autonomous Skills for Claude Code Agents
Skills are what transform agents from “helpful assistant” to “autonomous worker.” Without skills, you’re still babysitting every task. With the right skills, agents handle 70–80% of work without asking for guidance.
...
Time per review: 15 minutes → 3 minutes
...
I used to update docs manually. Then I’d forget for three weeks. Then onboarding a new developer revealed 47 outdated references. This skill watches for changes and queues doc updates.

Key metrics:

Doc accuracy: 65% → 94%
Time maintaining docs: 5 hours/week → 45 minutes
3. Test Generation Skill
What it does: Generates unit tests for new functions, edge case tests for complex logic.

What it doesn’t do: Replace human judgment on what to test. I learned this when the skill generated 127 tests for a 20-line function. Now it proposes tests; I approve or modify.
...
The biggest mistake I made: starting everything at Full Auto. I lost an afternoon reverting “improvements” the Refactoring Scout made to a critical path I’d optimized by hand. Now, any skill that modifies code starts at Supervised and earns autonomy over time.
...
The hooks section is critical. Before any review, I run lint and format checks — no point having an agent review code that won't pass CI. After, I log metrics (so I know if review quality degrades over time) and save context (so I can resume if interrupted).
...
Six months in, I’ve catalogued exactly where this setup falls apart. If you’re building something similar, learn from my mistakes.

Context Window Limits
The 141 agents work — until you try to coordinate more than 10 simultaneously. Beyond that threshold, context management becomes the bottleneck. I’ve measured this: accuracy drops from 89% to roughly 60% when juggling 15+ file modifications in a single session.
...
Even with explicit preservation rules, roughly 15% of context gets lost in handoffs between teams. Sometimes it’s nuance (“I chose approach A because of edge case X” becomes just “used approach A”). Sometimes it’s important constraints that get deprioritized.

My workaround: Critical decisions get written to a shared .claude/decisions.md file that every team reads.

Cost Accumulation
This system is not cheap. On heavy development days, I’ve hit $40 in API costs. Average is around $12/day with my usage patterns.

My workaround: The /costs command exists because I needed it. I set daily limits and get alerts at 80%.

Skill Drift
Skills that work perfectly today slowly degrade as codebases evolve. The Code Review skill flagged 62% more issues in month 1 than month 6 — not because code quality improved, but because it wasn’t keeping up with new patterns.

My workaround: Monthly skill audits. I set calendar reminders.

The Human Oversight Problem
Here’s the uncomfortable truth: the more autonomous the system becomes, the less I understand what it’s doing. Twice, I’ve discovered agents making “optimizations” that violated business logic they didn’t understand.
...
Start with 3–5 agents, not 20. I built too much infrastructure before validating the patterns.
Log everything from day one. I lost valuable debugging data in the first two months.
Define success metrics before building. “Is this agent helping?” is too vague. “Did review time decrease by 30%?” is actionable.
...
Use a team-based structure rather than task-based. Group agents by domain ownership: Engineering teams (Blue, Cyan, Orange, Yellow) own code areas, Support teams (Green, Red, Purple) handle quality and planning, Operational teams (Gray, White, Black) manage docs and incidents. Each team has 2–3 agents maximum with isolated contexts and explicit handoff rules. This prevents context bleed and coordination overhead.

How many agents should I start with?

Start with 3–5 agents in a single team before scaling. Most developers fail by building too many agents too fast — they create coordination complexity that negates productivity gains. A healthy progression: one team running smoothly for 2–3 weeks, then add a second. Scale based on measured time savings, not theoretical improvements.

What This System Actually Buys You
After 6 months with 141 agents, 8 skills, and 19 commands:

Time saved: 12–15 hours per week. Mostly in code reviews (3 hours), documentation (2 hours), and test writing (4 hours). The rest is scattered across debugging, deployment prep, and context switching.

Time invested: Initial setup took 45 hours over three weeks. Ongoing maintenance is 2–3 hours monthly.
...
New problems created: Cost management, skill maintenance, and the constant temptation to automate things that shouldn’t be automated.

Is it worth it? For my workflow — a CTO managing technical work across a 7-person direct team — yes. For a solo developer on small projects, probably overkill. For a team that ships 30+ PRs weekly, essential.

The full configuration lives in claude-code-tresor. Clone it, start with the Blue Team setup, and scale from there.

I’m still iterating on this system. Some days I think I’ve got it figured out. Other days, I discover another edge case that requires rethinking the architecture. If you build something similar, I’d genuinely like to know what breaks for you.

My Dedicated Claude Code Skills — A Claude Code Plugin and Claude AI Packages for all teams:

GitHub - alirezarezvani/claude-skills: A Collection of Skills for Claude Code and Claude AI for…
A Collection of Skills for Claude Code and Claude AI for real-world Usage. Including Claude Code Subagents, Claude Code…
github.com

✨ Thanks for reading! If practical insights on AI development tools are useful to you, hit subscribe for more production-tested patterns — no hype, just what actually works.
