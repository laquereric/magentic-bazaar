...
### 🎯 UML Diagram
**Type:** User Journey  
**File:** [7_Claude_Prod_Hook__user_journey__b2de9ed.puml](doc/uml/7_Claude_Prod_Hook__user_journey__b2de9ed.puml)
...
### 📋 Technical Summary
**File:** [7_Claude_Prod_Hook__b2de9ed.md](doc/skills/7_Claude_Prod_Hook__b2de9ed.md)
...
You’ve configured your PostToolUse hook to auto-format on every file write. It worked… once. Now Claude Code ignores it completely, and you’re back to manually running Prettier. Five GitHub issues, three Stack Overflow threads, and two hours later — you discover the config lives in the wrong settings.json file.

Press enter or click to view image in full size
7 Essential Claude Code Hooks for your Coding Workflow
7 Essential Claude Code Hooks for your Coding Workflow | Image Credits: © Gemini 3 Pro
...
Essential → Power User → Pro
Claude Code offers eight lifecycle events where hooks can execute. You don’t need all eight. Most production workflows run on three to four hooks total.
...
Hook #1 — PostToolUse: Auto-Format Everything
Runs immediately after Claude Code writes or edits any file. Eliminates “forgot to run Prettier” commits.
...
Hook #2 — Stop: Session Cleanup & Notifications
...
Hook #3 — PreToolUse: Safety Gates
...
Hook #4 — PostToolUse: Auto-Test on Every Edit
...
Tradeoff: Immediate feedback vs. development speed. Tests taking longer than 30 seconds disrupt flow — use background execution or move to Stop hook instead.

Hook #5 — SessionStart: Load Project Context
...
SessionStart output gets injected directly into Claude’s context. Claude sees git status and open issues before your first prompt.

This matters most for teams with strict conventions (commit messages, branch naming). Claude follows conventions automatically because they’re in context from session start.

Hook #6 — UserPromptSubmit: Input Validation & Enhancement
...
print(prompt)
sys.exit(0)
Use this sparingly — reserve for actual security/compliance requirements, not style preferences.

Pro Hook: Audit-Ready Logging
Hook #7 — All Events: Comprehensive Audit Trail
...
Daily log files average 5–10MB. Store on encrypted volumes for sensitive data.
...
Root cause: Claude Code reads from two locations — ~/.claude/settings.json (global) and .claude/settings.json (project-specific). Project settings completely override global settings. They don't merge.
...
Root cause: Exit code 2 has special meaning — it blocks execution and feeds stderr back to Claude. Exit codes 0 and 1 are non-blocking.
...
Custom slash commands, subagents, and automation workflows that transformed my team’s productivity — with copy-paste…
alirezarezvani.medium.com

Issue #4: Permission Review Skipped
Symptom: Hooks added to settings.json but show “pending review” forever. Never execute.

Root cause: Security feature — hooks require manual approval after configuration changes.
...
This approval step can’t be automated. If hooks auto-approved, malicious config files could execute arbitrary commands without user awareness.
...
If the log updates but your actual hook script doesn’t work, test the command directly in terminal:
...
10 Claude Code 2.0 techniques that turned a 3-week project into 3 days. Real production workflows from a CTO who saved…
alirezarezvani.medium.com
...
Week 2–3: Power User Tier (if applicable)
...
Hooks should be invisible — if you notice them running, they’re too slow
...
Which hook are you implementing first? Drop a comment with your workflow challenge — I’ll suggest the right hook pattern for your situation.
